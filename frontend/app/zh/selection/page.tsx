            ...
            {detail.reddit_posts.map((p, i) => (
              <a key={i} href={p.url} target="_blank" rel="noopener noreferrer"
                ...
                <div className="text-xs flex items-center gap-2 mt-0.5" style={{ color: "#555580" }}>
                  ...
                  }}>{p.sentiment === "positive" ? "正面" : p.sentiment === "negative" ? "负面" : "中性"}</span>
                </div>
              </a>
            ))}
          </div>
          ...
          <div className="space-y-1.5">
            {detail.youtube_kols.map((k, i) => (
              <a key={i} href={k.video_url} target="_blank" rel="noopener noreferrer"
                ...
                <div className="text-xs flex items-center gap-2 mt-0.5" style={{ color: "#555580" }}>
                  <span>{(k.subscribers / 1000).toFixed(0)}K 订阅</span>
                  <span>{(k.views / 1000).toFixed(0)}K 播放</span>
                  <span className="px-1 rounded" style={{
                    color: k.tier === "mega" ? "#ff6b35" : k.tier === "macro" ? "#f5a623" : "#7070a0",
                  }}>{k.tier === "mega" ? "头部" : k.tier === "macro" ? "中腰部" : "小微"}</span>
                </div>
  ...
  const skuMatches = q.length >= 2
    ? Object.entries(PRODUCT_DATA).flatMap(([catId, products]) => {
        ...
        return products
          ...
          .map(p => ({ ...p, catId, catName: (cat as { categoryZh?: string })?.categoryZh ?? catId }));
      })
    : [];
    ...
    <div className="space-y-10 py-12">
      ...
      {isDemo && !loading && (
        <div className="mx-auto max-w-3xl rounded-xl px-5 py-3 text-center text-sm"
          ...
          当前显示示例数据。<Link href="/zh/login" className="underline font-medium">登录</Link>查看实时 AI 可见度数据。
        </div>
      )}
      ...
      <div className="text-center space-y-4 max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-center gap-3 pt-1 text-xs" style={{ color: "#7070a0" }}>
          <span>{categories.length} 个品类</span><span>·</span>
          <span>4 大 AI 引擎</span><span>·</span>
          <span>Reddit + YouTube + Google Trends</span><span>·</span>
          <span style={{ color: "#22c55e" }}>{isDemo ? "示例" : "\u25CF 实时"}</span>
        </div>
      </div>
      ...
      <div className="max-w-xl mx-auto px-4">
        <div className="relative">
          <input
            ...
            placeholder="搜索品牌、产品或 ASIN（例如 NOCO GB40、Vantrue N4、B015TKUPIC...）"
        ...
        {q.length >= 2 && (
          <p className="text-xs mt-2 text-center" style={{ color: "#555580" }}>
            找到 {skuMatches.length + freeItems.length} 条结果 &ldquo;{q}&rdquo;
            {skuMatches.length > 0 && ` \u00b7 其中 ${skuMatches.length} 条产品级匹配`}
          </p>
        )}
      </div>
            ...
            <div key={i} className="rounded-xl p-4 space-y-2"
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="shrink-0 text-right">
                  ...
                  <div className="text-xs" style={{ color: "#7070a0" }}>AI 提及次数</div>
              ...
              <p className="text-xs leading-relaxed italic" style={{ color: "#555580" }}>{p.aiContextZh ?? p.aiContext}</p>
            </div>
          ))}
        </div>
      )}
            ...
            return (
              <div key={cat.id} className="rounded-xl p-5 space-y-4 flex flex-col"
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  ...
                  {cat.platforms.map(p => {
                    const ps = PLATFORM_STYLE[p] ?? { color: "#7070a0", bg: "rgba(112,112,160,0.08)" };
                    return (
                ...
                {cat.topBrands.length > 0 ? (
                  ...
                  <div className="text-xs py-4 text-center" style={{ color: "#3a3a5c" }}>
                    暂无扫描数据 — <Link href="/zh/scan" className="underline" style={{ color: "#ff6b35" }}>运行扫描</Link>获取该品类数据
                  </div>
                )}

                {/* Notes — show Chinese note */}
                <p className="text-xs leading-relaxed" style={{ color: "#7070a0" }}>{cat.sellerNoteZh || cat.sellerNote}</p>
                ...
                <div>
                  ...
                  >
                    {isDetailLoading ? (
                      ...
                      `\u25BC 查看详情${products.length > 0 ? ` + ${products.length} 款产品` : ""}`
                    )}
                  </button>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 rounded-2xl"
            ...
            <div className="text-center space-y-2 px-6 max-w-sm">
              ...
              <p className="text-xs" style={{ color: "#7070a0" }}>
                完整跨平台情报：AI 可见度、Reddit 舆情、YouTube 创作者覆盖和 Google Trends 动量。
              </p>
            </div>
      ...
      <div className="rounded-xl p-5 max-w-2xl mx-auto text-sm space-y-2"
        ...
        <div className="space-y-1 text-xs" style={{ color: "#7070a0" }}>
          ...
          <div><span style={{ color: "#ff4d6d" }}>GEO 评分 &ge; 50</span> = AI 可见度弱——有入场空间</div>
